...
public final class InfomationProcessor extends BaseProcess {
	...
	* @throws
	...
	private boolean infoBuilerByAcct(String tenantid,String ownertype,String ownerid) throws OmcException {
			...
			//获取客户信息
			Customer cust = customerService.getCustomer(tenantid, account.getCustomerId());
			if (null == cust){
				throw new OmcException("Information","没有取道客户信息infoOwner"
						+ paramsString(tenantid,ownertype,ownerid));
			}
	...
	* @throws OmcException
	...
	private boolean infoBuilerByUser(String tenantid,String ownertype,String ownerid) throws OmcException{
		...
		if (null == user){
			throw new OmcException("Information","没有取道用户信息infoOwner:"
					+ paramsString(tenantid,ownertype,ownerid));
		}
		...
		if (null == account ){
			throw new OmcException("Information","没有取道账户信息infoOwner:"
					+ paramsString(tenantid,ownertype,ownerid));
		}
